...
class data_loader(Dataset):
    ...
    def __getitem__(self, item):
        ...
        block_list = literal_eval(self.data['block'][item])
        ...
        print(sentence)
        output_tokenizer = self.tokenizer(block_list, max_length=self.max_len_block, padding='max_length')
        input_ids = tensor(output_tokenizer['input_ids'])
        attention_mask = tensor(output_tokenizer['attention_mask'])
        block_bert_feature = self.bert_model(input_ids=input_ids, attention_mask=attention_mask)
        ...
        return {'sentence_bert_feature': sentence_bert_feature['last_hidden_state'][0][0].to(device),
                'block_bert_feature': block_bert_feature['last_hidden_state'][:, :1, :].squeeze().to(device),
