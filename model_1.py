...
class back_channel_sentence_1(torch.nn.Module):
    def __init__(self, len_block:int, batch_size:int):
        ...
        self.lstm_sentence = torch.nn.LSTM(
            input_size = 768,
            hidden_size = 128,
            num_layers = 2,
            batch_first = True
        )
        self.lstm_relation = torch.nn.LSTM(
            input_size=128,
            hidden_size=64,
            num_layers=2,
            batch_first=True
        ...
        self.full_connection = torch.nn.Linear((1+self.len_block)*64, 2)
    ...
    def forward(self, bert_feature_sentence, bert_feature_block):
        ...
        x_input = x_input.reshape(x_input.size(0), (1+self.len_block)*64)
...
def train_model(model:back_channel_sentence_1, data_loader):
    ...
    optimizer = torch.optim.Adam(params=model.parameters(), lr=1e-05, weight_decay=10)
    epoch = 5
    ...
    for num in range(epoch):
        ...
        for index, real_data in enumerate(data_loader):
            ...
            with open('judge_result_11_5_'+str(model.len_block)+'.txt', 'a+') as file:
...
def valid_model(model:back_channel_sentence_1, data_loader):
    ...
    for index, real_data in enumerate(data_loader):
        ...
        amount_all += len(target)
        print(n_correct / len(target))
        with open('judge_model_11_5_'+str(model.len_block)+'test.txt', 'a+') as file:
            file.write(str(n_correct / len(target)) + '\n')
    print('\033[1;32;46m' + str(amount_correct / amount_all) + '\033[0m')
    print(amount_correct / amount_all)
    with open('judge_model_11_5_'+str(model.len_block)+'test.txt', 'a+') as file:
